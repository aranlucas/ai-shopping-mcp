...
`agents/api/openapi/grocery-gateway.yaml`: `oapi-codegen` generates the Go server/client and
`openapi-typescript` generates this Worker's client. Agent runtime-state contracts are a
separate, Go-canonical system generated by `cmd/contracts`. After changing the gateway spec,
run `pnpm --filter ai-shopping-list-mcp generate:gateway` and
`pnpm --filter ai-shopping-list-mcp api:check`.
