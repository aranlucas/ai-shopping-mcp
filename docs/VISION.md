...
1. **The hands — this repo.** A Cloudflare Worker MCP server: Kroger/QFC API access, OAuth,
   cart retry state, weekly-deal scraping, and a tool surface engineered for weak models.
...
Non-Kroger persistent data (preferred store, pantry, equipment, orders, and shopping lists)
is owned by agents-gateway/D1 and shared with the user's agents household library. The Worker
accesses it through `/api/grocery/*`; user-scoped KV remains for Kroger cart retry state, MCP
transport state, and the product/location cache. The HTTP wire contract is spec-canonical in
`openapi/grocery-gateway.yaml`: `oapi-codegen` generates the Go server/client in agents
and `openapi-typescript` generates this Worker's client. After changing the gateway spec,
run `pnpm generate:gateway` and `pnpm api:check`.

Catalog identity is provider-neutral at every shared boundary. A product is identified by a
`ProductReference` (`provider` plus the provider-scoped `id`), rendered to small models as the
copyable `productRef=<provider>:<id>` token. UPC, SKU, and future identifiers are adapter details,
not fields in the universal model. Provider-specific capabilities stay explicit: a catalog may
support search without supporting carts, locations, aisle data, or checkout. The registry is
open-ended, and generic tools discover its providers at registration time rather than compiling a
closed retailer enum. Provider-specific tools may still expose native identifiers when the
upstream operation requires one (for example, Kroger cart writes require a UPC).
...
- **Aisle data is opt-in.** `search_products` returns `aisleLocations` and prints aisle,
  ...
  `includeLocation=true`. This keeps ordinary product searches compact while supporting
