...
export class MyMCP extends McpAgent<Env, unknown, Props> {
  ...
  async init() {
    ...
    // Plan meals tool - AI-powered meal planning from pantry
    this.server.registerTool(
      "plan_meals",
      {
        description:
      ...
      async ({
        ...
        const [pantry, equipment, recentOrders] = await Promise.all([
          storage.pantry.getAll(this.props.id),
          storage.equipment.getAll(this.props.id),
          storage.orderHistory.getRecent(this.props.id, 10),
        ]);
