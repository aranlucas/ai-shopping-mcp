...
- Cache only product and location responses. Never put cart or identity responses in the shared cache. Kroger cart retry state stays in user-scoped Cloudflare KV; all non-Kroger persistence lives in agents-gateway/D1 behind `agents/api/openapi/grocery-gateway.yaml`.
- After changing the gateway contract, regenerate and verify the Worker client with `pnpm --filter ai-shopping-list-mcp generate:gateway` and `pnpm --filter ai-shopping-list-mcp api:check`.
